...
@end
/// Rich media adapter protocol delegate for rendering rich media ad
@protocol BDMRichMediaAdapterDisplayDelegate <BDMAdapterDisplayDelegate>
/// Return nonnul root view controller for presenting product page
/// and tracking viewability
/// @param adapter Current presentig adapter
/// @return Nonnul instance root view controller that view is superview of media view
- (UIViewController *)rootViewControllerForAdapter:(id<BDMRichMediaAdapter>)adapter;
/// Called when user tap on media views
/// @param adapter Current presentig adapter
- (void)adapterRegisterUserInteraction:(id<BDMRichMediaAdapter>)adapter;
/// Called when adapter open product link in external
/// browser (Safari) after user interaction
/// @param adapter Adapter that revieve user interaction
- (void)adapterWillLeaveApplication:(id<BDMRichMediaAdapter>)adapter;
/// Called before adapter open product link in StoreKit or Safari
/// view controller internally in application after user interaction
/// @param adapter  Adapter that revieve user interaction
- (void)adapterWillPresentScreen:(id<BDMRichMediaAdapter>)adapter;
/// Called after adapter dismiss product link in StoreKit or Safari
/// view controller internally in application after user interaction
/// @param adapter Adapter that revieve user interaction
- (void)adapterDidDismissScreen:(id<BDMRichMediaAdapter>)adapter;
...
@end
/// Banner adapter protocol for rendering rich media
@protocol BDMRichMediaAdapter <BDMAdapter>
/// Callback handler
@property (nonatomic, weak, nullable) id <BDMRichMediaAdapterDisplayDelegate> displayDelegate;
/// Call this method to start rendering rich media ad
/// @param container Container for presenting ad view
- (void)presentInContainer:(UIView *)container;
...
/// Fire method when network will tracked close view
- (void)nativeAdAdapterTrackFinish:(id<BDMNativeAdAdapter>)adapter;
/// Fire method when network will tracked impression view
- (void)nativeAdAdapterTrackImpression:(id<BDMNativeAdAdapter>)adapter;
/// Fire method when network will tracked viewability
- (void)nativeAdAdapterTrackViewability:(id<BDMNativeAdAdapter>)adapter;
...
- (void)unregisterView;
/// Is called when native will be clicked
